...
- **文风与模板感：** 依据本书文风契约，修订重复解释、同质对白、泛化描写和套路化收尾，并比较相邻章节中的重复习惯。
- **跨章实体连续性：** 按稳定实体标识与正文来源核对身份、属性和当前状态，区分别称、误认、正常变化与矛盾；有事件依据的变化保留历史。

这些是写前约束与写后审稿机制，不能保证零错误或某种 AI 检测结果。脚本负责验证审稿记录和提交条件，无法理解对白、判断文风或自动识别全部语义冲突。
